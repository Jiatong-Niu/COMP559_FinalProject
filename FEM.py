...
Node_list = np.array([[0, 0], [1, 0], [0, 0.5]])
Element_list = np.array([[1, 2], [2, 3], [3, 1]])

# assume first node is fixed in all direction, second node is fixed in y direction
...
# ENL is used to store all the information
ENL = np.zeros([NoN, 6 * PD])

ENL[:, 0:PD] = Node_list[:, :]
ENL[:, PD:2 * PD] = Boundary[:, :]

# Get information of BC
...
for scale in range(0, 20):
    ## here we basicly use scale to show the effect of the displacement
